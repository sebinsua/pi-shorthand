#!/bin/sh
# Runs the test suite on Linux, in a Debian container with Node 24 and bubblewrap. Needs Docker.
...
set -e
cd "$(dirname "$0")/.."
docker run --rm --privileged --tmpfs /tmp:exec -v "$PWD":/src:ro node:24-trixie sh -c '
	set -e
